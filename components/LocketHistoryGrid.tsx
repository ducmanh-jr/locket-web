...
export const LocketHistoryGrid: React.FC<LocketHistoryGridProps> = ({
  ...
  return (
    <div className="w-full h-full bg-[#0d0615]/90 backdrop-blur-2xl text-white flex flex-col justify-between p-4 pt-3 pb-24 overflow-y-auto custom-scrollbar select-none">
      {/* Top Header Bar */}
      <div className="flex items-center justify-between pb-3 border-b border-white/10 flex-shrink-0">
        <div className="flex items-center space-x-2 bg-black/40 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10">
          <Grid className="w-4 h-4 text-[#FF2A85]" />
          <h1 className="text-white text-xs font-black tracking-wider uppercase flex items-center gap-1.5">
            Lịch sử khoảnh khắc
            <span className="text-[10px] bg-[#FF2A85] text-white px-2 py-0.5 rounded-full font-bold">
              {moments.length}
            </span>
          </h1>
        </div>
            ...
            {moments.map((moment, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className="relative aspect-square rounded-2xl overflow-hidden bg-[#180d1a] border border-white/10 cursor-pointer group transform-gpu shadow-lg hover:border-[#FF2A85]/50 transition-colors"
                >
                  ...
                  {isVideo && (
                    <div className="absolute top-1.5 right-1.5 bg-black/70 backdrop-blur-sm text-white text-[9px] font-bold px-1.5 py-0.5 rounded-md flex items-center space-x-0.5 border border-white/15">
                      <Video className="w-3 h-3 text-[#FF2A85]" />
                    </div>
                  )}
                </motion.div>
              );
